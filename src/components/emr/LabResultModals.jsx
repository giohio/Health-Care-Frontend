...
const PRIORITY_STYLES = {
  ...
  priority: { bg: 'bg-amber-50 dark:bg-amber-950/30',   border: 'border-amber-200 dark:border-amber-700/40',  text: 'text-amber-700 dark:text-amber-400',  dot: 'bg-amber-500',  label: 'PRIORITY' },
...
export function AiDraftRenderer({ text, fileType, collapsed = false }) {
  if (!text) {
    if (fileType === 'manual') {
      return <p className="text-xs italic opacity-60 text-amber-600 dark:text-amber-400">Manual entries submitted — upload again to rerun AI with updated source data.</p>
    }
...
const REQUIRED_CSV_FIELDS = new Set(['test_name', 'value'])
const CSV_FIELD_ALIASES = {
  test_name: ['test_name', 'test', 'name', 'parameter', 'analyte', 'chỉ_số', 'chi_so'],
  value: ['value', 'result', 'result_value', 'kết_quả', 'ket_qua', 'giá_trị', 'gia_tri'],
  unit: ['unit', 'units', 'uom', 'đơn_vị', 'don_vi'],
  reference_range: ['reference_range', 'ref_range', 'range', 'normal_range', 'reference', 'ref', 'khoảng_tham_chiếu', 'khoang_tham_chieu'],
  flag: ['flag', 'abnormal', 'status', 'cờ', 'co'],
}
...
function findCsvField(headers, field) {
  const aliases = CSV_FIELD_ALIASES[field] || [field]
  return headers.find((header) => aliases.includes(header.key))?.key || ''
}

function normalizeLabFlag(value) {
  const normalized = String(value ?? '').trim().toLowerCase()
  if (!normalized) return ''
  if (['n', 'normal', 'negative', 'neg', 'within_range', 'within_normal_range'].includes(normalized)) return 'N'
  if (['h', 'high', 'positive', 'pos', 'above', 'above_range', 'abnormal_high'].includes(normalized)) return 'H'
  if (['l', 'low', 'below', 'below_range', 'abnormal_low'].includes(normalized)) return 'L'
  if (['c', 'critical', 'critical_high', 'critical_low', 'panic'].includes(normalized)) return 'C'
  return ''
}

function buildManualEntryFromCsvRow(row, headers) {
  const byField = {
    test_name: findCsvField(headers, 'test_name') || headers[0]?.key || '',
    value: findCsvField(headers, 'value') || headers[1]?.key || '',
    unit: findCsvField(headers, 'unit'),
    reference_range: findCsvField(headers, 'reference_range'),
    flag: findCsvField(headers, 'flag'),
  }

  const entry = {}
  headers.forEach((header) => {
    const value = String(row[header.key] ?? '').trim()
    if (value) entry[header.key] = value
  })

  ;['test_name', 'value', 'unit', 'reference_range', 'flag'].forEach((field) => {
    const sourceKey = byField[field]
    const value = sourceKey ? String(row[sourceKey] ?? '').trim() : ''
    if (field === 'flag') {
      const flag = normalizeLabFlag(value)
...
// Parse a CSV/TSV text using the first row as dynamic headers.
function parseCsv(text) {
  ...
  const headers = rawHeaders.map((label, index) => {
    ...
    const baseKey = normalizeCsvHeader(label) || fallback
  ...
  const rows = lines.slice(1)
    .map((line) => {
      ...
      return headers.reduce((row, header, index) => ({
        ...row,
        [header.key]: cols[index] ?? '',
      }), { _id: crypto.randomUUID() })
    })
    .filter((row) => headers.some((header) => String(row[header.key] ?? '').trim()))

  return { headers, rows }
}

export function UploadModal({ order, resultId, onClose, onSuccess }) {
  ...
  const handleCsvFileChange = (e) => {
    ...
    reader.onload = (ev) => {
      ...
      if (parsed.headers.length === 0) { setError('No header row found in CSV.'); return }
  ...
  const addCsvRow = () => setCsvRows((prev) => [
    ...prev,
    csvHeaders.reduce((row, header) => ({ ...row, [header.key]: '' }), { _id: crypto.randomUUID() }),
  ])
  const removeCsvRow = (idx) => setCsvRows((prev) => prev.filter((_, i) => i !== idx))

  const handleCsvSubmit = async () => {
    const validRows = csvRows
      .map((row) => buildManualEntryFromCsvRow(row, csvHeaders))
      .filter((entry) => String(entry.test_name ?? '').trim() && String(entry.value ?? '').trim())
    ...
    try {
      const payload = {
        ...
        manual_entries: validRows,
        notes: csvNotes || undefined,
      }
      if (resultId) await emrApi.replaceLabResultInput(resultId, { ...payload, file_type: 'manual' })
      else await emrApi.createLabResultManual(payload)
      onSuccess()
    } catch (err) {
  ...
  const handleFileSubmit = async () => {
    ...
    try {
      ...
      if (resultId) await emrApi.replaceLabResultInput(resultId, payload)
      else await emrApi.createLabResultFile(payload)
      onSuccess()
    } catch (err) {
            ...
            <div className="space-y-3">
              ...
              {csvRows.length === 0 ? (
                <button type="button" onClick={() => csvInputRef.current?.click()}
                  ...
                  <span className="text-xs">CSV or TSV — first row is used as the table header</span>
                </button>
              ) : (
                <>
                  <div className="flex items-center justify-between">
                    <p className="text-xs font-medium text-slate-600 dark:text-[#9898b0]">{csvFileName} — {csvRows.length} rows</p>
                    <button type="button" onClick={() => { setCsvRows([]); setCsvHeaders([]); setCsvFileName(null); csvInputRef.current && (csvInputRef.current.value = '') }}
                      className="text-xs text-slate-400 hover:text-rose-500">Clear</button>
                  </div>
                  <p className="text-[11px] text-slate-400 dark:text-[#606070]">
                    Submit requires a test/name column and a value/result column. Unknown columns are kept with the row data.
                  </p>
                  <div className="max-h-60 overflow-auto rounded-xl border border-slate-200 dark:border-[#252530]">
                    <table className="min-w-full text-left text-xs">
                      <thead className="sticky top-0 bg-slate-50 dark:bg-[#16161e]">
                        <tr>
                          {csvHeaders.map((header) => {
                            const testNameKey = findCsvField(csvHeaders, 'test_name') || csvHeaders[0]?.key
                            const valueKey = findCsvField(csvHeaders, 'value') || csvHeaders[1]?.key
                            const isRequired = header.key === testNameKey || header.key === valueKey
                            return (
                              <th key={header.key} className="min-w-[120px] px-2 py-2 font-semibold uppercase tracking-wide text-slate-400 dark:text-[#505060]">
                      ...
                      <tbody>
                        {csvRows.map((row, idx) => (
                          <tr key={row._id} className="border-t border-slate-100 dark:border-[#1c1c25]">
                            {csvHeaders.map((header) => (
                              <td key={header.key} className="px-2 py-1.5">
                                <input
                                  value={row[header.key] ?? ''}
                                  onChange={(e) => updateCsvRow(idx, header.key, e.target.value)}
                                  placeholder={header.label}
                                  className="w-full rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs text-slate-700 placeholder-slate-300 focus:border-indigo-400 focus:outline-none dark:border-[#252530] dark:bg-[#16161e] dark:text-[#c8c8e0] dark:placeholder-[#505060]"
...
function parseRawInputEntries(result) {
  if (Array.isArray(result.manual_entries)) return result.manual_entries
  if (Array.isArray(result.manualEntries)) return result.manualEntries
  const raw = result.raw_input_json || result.rawInputJson
  if (!raw) return []
  try {
...
function RawEntriesTable({ entries }) {
  ...
  const orderedColumns = [
    ...
    'reference_range',
    'flag',
    ...columns.filter((key) => !['test_name', 'value', 'unit', 'reference_range', 'flag'].includes(key)),
  ].filter((key, index, arr) => arr.indexOf(key) === index)

  return (
    <div className="overflow-x-auto rounded-xl border border-slate-100 bg-slate-50 dark:border-[#1c1c25] dark:bg-[#16161e]">
      <table className="w-full min-w-[520px] text-xs">
        <thead>
          <tr className="border-b border-slate-100 text-slate-400 dark:border-[#1c1c25] dark:text-[#505060]">
            {orderedColumns.map((key) => (
              <th key={key} className="px-3 py-2 text-left font-semibold uppercase tracking-wide">
                {key.replace(/_/g, ' ')}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {entries.map((entry, idx) => (
            <tr key={entry?._id || idx} className="border-b border-slate-100 last:border-0 dark:border-[#1c1c25]">
              {orderedColumns.map((key) => (
                <td key={key} className="px-3 py-2 text-slate-700 dark:text-[#c8c8e0]">
                  {String(entry?.[key] ?? '—')}
                </td>
              ))}
            </tr>
...
export function VerifyModal({ order, result, onClose, onConfirm }) {
  ...
  const entries = parseRawInputEntries(result)
  const fileUrl = result.file_url || result.fileUrl || ''
  const fileType = result.file_type || result.fileType || ''
          ...
          {entries.length > 0 && (
            <div>
              <div className="mb-2 flex items-center justify-between gap-3">
                <p className="text-xs font-semibold uppercase tracking-wide text-slate-400 dark:text-[#505060]">Raw Source Values</p>
                <span className="rounded-md bg-slate-100 px-2 py-0.5 text-[10px] font-semibold text-slate-500 dark:bg-[#1c1c25] dark:text-[#7070a0]">
                  {fileType || 'manual/csv'}
                </span>
              </div>
              <RawEntriesTable entries={entries} />
            </div>
          )}

          {entries.length === 0 && fileUrl && (
            <div className="rounded-xl border border-slate-100 bg-slate-50/60 px-4 py-3 dark:border-[#1c1c25] dark:bg-[#16161e]">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-[#505060]">Raw Source File</p>
                  <p className="mt-1 text-xs text-slate-500 dark:text-[#7070a0]">{fileType || 'uploaded file'}</p>
                </div>
                <a
                  href={fileUrl}
                  target="_blank"
                  rel="noreferrer"
                  className="rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-1.5 text-xs font-semibold text-indigo-700 transition hover:bg-indigo-100 dark:border-indigo-800/50 dark:bg-indigo-950/30 dark:text-indigo-300"
                >
                  View Raw File
                </a>
              </div>
